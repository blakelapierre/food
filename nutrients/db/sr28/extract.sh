...
copy() {
     ...
     docker run --rm -it \
               ...
               postgres:alpine /bin/bash -c "$COMMAND"
}
...
copy 'SELECT json_build_array(\"Nutr_No\", \"Units\", \"Tagname\", \"NutrDesc\", \"Num_Dec\", \"SR_Order\") FROM \"sr28\".\"NUTR_DEF\" ORDER BY \"SR_Order\"' 'nutr_def/data.txt'
